## 2026-09-15 · نسخه‌ی 2.0.4 — همه‌ی متن‌ها با کد یکی شدند: ۱۵ ثانیه، جریمه‌ی پله‌ای، باتری خزنده

- **ساعت گیرکردن ۱۵ ثانیه است، نه ۱۰.** کارت قوانین FS و U14، جدول «Match
  rules» داخل بازی و پیام داور («۱۵ ثانیه گیر کرده بود») همه با
  `STUCK_SECONDS = 15` و `STUCK_RADIUS = 0.85` هماهنگ شدند.
- **جابه‌جایی = کاشی آزادِ تصادفی در هر جای خانه، دستِ‌کم ۶ کاشی آن‌طرف‌تر** —
  «چند کاشی آن‌طرف‌تر» از همه‌ی متن‌ها برداشته شد. جریمه در هر سه رده پله‌ای
  است: **−۵ / −۱۰ / −۱۲ کاشی**، و جابه‌جاییِ بدون جریمه وجود ندارد.
- **باتری صفر نمی‌کشد، می‌خزد (۱۵٪ سرعت):** `RULES.md`، `CODING.md`،
  `leagues/vacuum/README.md`، راهنمای داخل بازی، دو قالبِ کد شروع داخل
  `index.html`، گزارش پایان مسابقه و هدر ربات قهرمان U19 اصلاح شدند.
- **مخزن خاک در U19 خاموش است** (`dump: false`) — حالا در `RULES.md`، کارت
  قوانین U19 و راهنمای داخل بازی صریح نوشته شده که ایستگاه تخلیه در
  مسابقه‌های امسال وجود ندارد؛ متن قانون برای روزِ روشن شدنش نگه داشته شد.
- **سایت:** صفحه‌های `/rules` و `/fa/ghavanin` بخش تازه‌ی «جابه‌جایی، جریمه و
  پایان مسابقه» گرفتند و بخش باتری با عددهای واقعی (۶۰ ثانیه رانندگی،
  ۲۵٪ شارژ در ثانیه، خزیدن ۱۵٪، آب −۱۰٪) بازنویسی شد؛ اعلان انتشار نسخه‌ی
  تازه با تاریخ ۲۴ شهریور ۱۴۰۵ روی صفحه‌های دانلود نشست.
- EN: every rule text now matches the engine — a 15 s watchdog, a random
  relocation at least 6 tiles away, the climbing 5/10/12 fine, a flat battery
  that limps at 15 % instead of dying, and the U19 dust bin declared off.

## 2026-09-10 (d) — U19 gets its own room: stone, dark walls, Persian carpets, water

- **فقط U19** عوض شد (FS و U14 مو به مو مثل قبل‌اند — با تست تأیید شد):
  کاشی‌ها سنگ تیره‌ی درزباریک به‌جای گریدِ روشن، دیوارها تقریباً مشکی، پالت
  جدی و بدون رنگ پاستلی.
- **فرش‌های ایرانی:** فرش‌های U19 با بافت ایرانی رندر می‌شوند (حاشیه‌ی قلاب‌دار،
  ترنجِ کنگره‌دار با آویز، لچکِ گوشه‌ها، بته‌جقه و ریشه) — بافت روی کانواس ساخته
  می‌شود، پس هیچ فایل تصویری اضافه نشد. رنگِ زمینه از خودِ فرش می‌آید، پس
  «فرش سبز» هنوز سبز است و درسِ سنسور رنگ سر جایش می‌ماند.
- **آب روی زمین (فقط U19):** چهار لکه‌ی آب روی خطوط کاشی، از ثانیه‌ی اول دیده
  می‌شود؛ هر ورود تازه **−۱۰٪ باتری** (نه کاشی) و پیام «💦 وارد آب شد». مسیر
  درست، دور زدن با مختصات است — مختصات لکه‌ها در راهنمای داخل بازی و RULES.md
  آمده. `rules.waterBattery` این کار را می‌کند؛ هر رده‌ای که این کلید را نداشته
  باشد دقیقاً مثل قبل رفتار می‌کند.
- EN: a U19-only theme (dark stone tiles, near-black walls, serious palette),
  Persian carpets drawn procedurally, and water that costs 10% battery per
  entry — visible from the first second, avoided by position.

## 2026-09-10 (c) — map 6: the open four-bedroom house

- **مپ ۶** (`organizer-only/maps/map6/`): **۲۶×۲۶** (بزرگ‌ترین زمین تا حالا،
  ۶۲۶ کاشی آزاد)، هال صلیبی، **۴ اتاق‌خواب** در چهار گوشه، آشپزخانه و سرویس در
  نوارهای میانی. همه‌ی ورودی‌ها **۴ کاشی (۲٫۵ متر)**، **هیچ دری** و **هیچ فرش
  نشانه‌ی بنفشی** — عمداً بیرون از استاندارد، به خواست برگزارکننده، تا رفتن به
  اتاق‌ها آسان و امتیازها بالا باشد. تست: مجموع امتیاز دو تیم ۲۴۴ تا ۲۷۵
  (روی مپ ۳ همان ربات‌ها ۱۹۰)؛ اعتبارسنج: هر شش اتاق ۱۰۰٪ قابل‌دسترس.
- **اسلات ششم:** بازی، مپ‌ساز، `tools/mapserver.py`، سرور سایت و پنل ادمین همه
  تا مپ ۶ می‌شمارند؛ بازی هم `map6.json` کنار خودش را پیدا می‌کند. اتاق ششم
  یعنی `clean6` هم به برنامه داده می‌شود. `make-map-kit.py` حالا پوشه‌ها را
  می‌خواند، پس مپ بعدی هیچ ویرایشی لازم ندارد.
- EN: map 6 — a 26x26 open four-bedroom house with 4-tile doorways, no door
  and no marker rugs (a recorded exception to the map standard); six slots
  everywhere and clean6 for the sixth room.
